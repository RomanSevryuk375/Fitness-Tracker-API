...
public class PhotoRepository(FitnessDbContext context) : IPhotoRepository
{
    ...
    public async Task<Photo?> GetByFilePathAsync(string filePath, CancellationToken cancellationToken = default)
    { 
        return await context.Photos
            .FirstOrDefaultAsync(p => p.FilePath == filePath, cancellationToken);
    }

    public async Task<List<Photo>> GetByWorkoutIdAsync(Guid workoutId, CancellationToken cancellationToken = default)
    {
        return await context.Photos
            .AsNoTracking()
